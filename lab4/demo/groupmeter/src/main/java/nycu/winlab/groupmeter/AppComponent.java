            ...
            if (ethPkt.getEtherType() == Ethernet.TYPE_ARP) {
                ...
                if (arp.getOpCode() == ARP.OP_REQUEST) {
                    ...
                    if (targetIp.equals(Ip4Address.valueOf(config.getIp1()))) {
                        ...
                        packetService.emit(
                                new DefaultOutboundPacket(deviceId, treatment, ByteBuffer.wrap(ethReply.serialize())));
                    } else if (targetIp.equals(Ip4Address.valueOf(config.getIp3()))) {
                        MacAddress targetMac = MacAddress.valueOf(config.getMac3());
                        Ethernet ethReply = ARP.buildArpReply(targetIp, targetMac, ethPkt);
                        TrafficTreatment treatment = DefaultTrafficTreatment.builder()
                                .setOutput(inPort).build();
                        context.block();
        ...
        public void event(NetworkConfigEvent event) {
            if ((event.type() == CONFIG_ADDED || event.type() == CONFIG_UPDATED) &&
                ...
                if (newconfig != null) {
                    ...
                    // Log
                    log.info("ConnectPoint_h1: {}, ConnectPoint_h2: {}, ConnectPoint_h3: {}", config.getHost1(),
                            config.getHost2(), config.getHost3());
                    log.info("MacAddress_h1: {}, MacAddress_h2: {}, MacAddress_h3: {}", config.getMac1(),
                            config.getMac2(), config.getMac3());
                    log.info("IpAddress_h1: {}, IpAddress_h2: {}, IpAddress_h3: {}", config.getIp1(), config.getIp2(),
                            config.getIp3());
                    ...
                    // ========================= Install Intent =========================
                    // h1 -> h3
                    ConnectPoint ingressPoint = ConnectPoint.deviceConnectPoint(config.getHost1());
                    ConnectPoint egressPoint = ConnectPoint.deviceConnectPoint(config.getHost3());

                    TrafficSelector trafficSelector = DefaultTrafficSelector.builder()
                            .matchEthDst(MacAddress.valueOf(config.getMac3()))
                            .build();
                    ...
                    // h3 -> h1
                    ingressPoint = ConnectPoint.deviceConnectPoint(config.getHost3());
                    egressPoint = ConnectPoint.deviceConnectPoint(config.getHost1());
                    ...
                    // h2 -> h3
                    ingressPoint = ConnectPoint.deviceConnectPoint(config.getHost2());
                    egressPoint = ConnectPoint.deviceConnectPoint(config.getHost3());
                    ...
                    // h3 -> h2
                    ingressPoint = ConnectPoint.deviceConnectPoint(config.getHost3());
                    egressPoint = ConnectPoint.deviceConnectPoint(config.getHost2());
